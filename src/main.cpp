...
// Window title
const char *windowTitle = "Program to test DIMG";
// Window pointer
GLFWwindow *window;

// Shader object
Shader *shader, *dsShader;
...
bool init()
{
	...
	dsShader = new Shader("assets/shaders/dsShader.vert", "assets/shaders/dsShader.frag");
    // Loads all the geometry into the GPU
    buildGeometry();
    // Loads the texture into the GPU
    textureID = api.loadImage("assets/textures/bricks2.jpg");
...
void render()
{
	...
	//if (dsTexture == 0)
	//	dsTexture = api.negative(imageID);
	//std::cout << "id: " << dsTexture << std::endl;
...
{
	// Init api
	api.setArrayAndBufferObjects(VAO, VBO);
    // Initialize all the app components
    if (!init() /*|| !setFrameBuffer()*/)
    {
